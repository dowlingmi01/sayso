 ...
 *
 */
class Game_Starbar_Music extends Game_Starbar {
	...
	protected function _visitGood (Gaming_BigDoor_Good $good, $quantity = 1)
	{
		...
		if ($good->getId() == $this->_economy->getGoodId('MONTH_ONE_TOKEN_BLAH_BLAH') || $good->getId() == $this->_economy->getGoodId('MONTH_ONE_TOKEN_MOO_MOO')) {
		...
		*/
			...
			if (parent::$profileSurveyId) $profileSurveyLink = '<a href="//'.BASE_DOMAIN.'/starbar/music/embed-survey?survey_id='.parent::$profileSurveyId.'" class="sb_nav_element" rel="sb_popBox_surveys_hg" title="Take profile survey now!" style="position: relative; top: -5px;">Profile Survey</a>';
			...
			$good->setNonRedeemReason('Earn more Music PaySos by<br />completing polls and surveys!');
			$good->setCommentForUser('Insufficient Music PaySos');
		}
