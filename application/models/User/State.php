...
class User_State extends Record
{
	...
	public function getStarbarList( $user_id ) {
		...
		$sql  = "SELECT s.id, s.short_name, s.label, s.info, sm.active FROM starbar s LEFT JOIN starbar_user_map sm ON (s.id = sm.starbar_id AND sm.user_id = ?) WHERE s.id > 1 ORDER BY active DESC";
		$result = array();
