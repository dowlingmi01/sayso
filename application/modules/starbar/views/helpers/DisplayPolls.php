			...
			foreach ($polls as $survey) {
				...
				<? if ($status == 'new' || $status == 'archived') { // User can take this poll ?>
					<h3>
						...
						<a href="//<?= BASE_DOMAIN ?>/starbar/<?= ($starbar->id == 3 ? "content" : $starbar->short_name) ?>/embed-poll?survey_id=<?= $survey->id ?>" rel="starbar-poll-<?= $survey->id ?>" iframeHeight="<?= $iframeHeight ?>">
