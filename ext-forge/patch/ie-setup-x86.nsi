...
Section 
    ...
    installation:       
        ...
        WriteRegStr   HKLM "${ARP}" "DisplayVersion"   "2.0.4"
    ...
    ie9: 
        ...
        File launchIE.exe
        ; The following line requires the Version plugin http://nsis.sourceforge.net/Version_plug-in
        Version::GetWindowsVersion
        Pop $R0
        ${If} $R0 < 6
            Exec '"$PROGRAMFILES\Internet Explorer\iexplore.exe" http://${SAYSO_BASE_DOMAIN}/starbar/install/post-install'
        ${Else}
            Exec '"$INSTDIR\launchIE.exe" http://${SAYSO_BASE_DOMAIN}/starbar/install/post-install'
        ${EndIf}
