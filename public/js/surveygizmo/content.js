(function () {
	setTimeout(function () { // Small delay to give page (and $SGQ variable) a chance to load
		...
		$SQ('head').append( $SQ('<link rel="stylesheet" type="text/css" />').attr('href', '//' + $SGQ.base_domain + '/css/surveygizmo/surveys-' + $SGQ.starbar_short_name + '.css') );
		...
		function everythingIsLoaded() {
			if (window.$SQ // sayso jQuery
				...
				&& ((!sayso.in_iframe && window.sayso.starbar.loaded) || sayso.in_iframe) // if loading outside iframe, make sure starbar has loaded
