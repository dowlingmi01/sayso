		...
		function afterSQloads () {
			...
			$.doTimeout('waitForEverything', timeBetweenChecks, function() {
			    if (everythingIsLoaded()) {
					...
					var redirectParameters = "?srid=" + $SGQ.srid + "&next_survey_id=" + $SGQ.next_survey_id + "&user_id=" + $SQ.sayso.starbar.user.id + "&user_key=" + $SQ.sayso.starbar.user.key + "&starbar_id=" + $SQ.sayso.starbar.id + "&auth_key=" + $SQ.sayso.starbar.authKey + "&" + easyXDMParameters + "#" + easyXDMParameters;
